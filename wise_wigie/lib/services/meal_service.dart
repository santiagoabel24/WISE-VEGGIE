...
class MealService {
  ...
  Stream<List<MealRecord>> getTodayMeals() {
    ...
    return _mealsRef
        ...
        .orderBy('date')
        .snapshots()
        .map((snap) => snap.docs.map((d) => MealRecord.fromFirestore(d)).toList());
  }

  // ── READ — Registros de los últimos 7 días ──
  Stream<List<MealRecord>> getWeekMeals() {
    final now           = DateTime.now();
    final startOfToday  = DateTime(now.year, now.month, now.day);
    // 7 días atrás desde el inicio de hoy
